...
use trustify_entity::{
    ...
    sbom_node_checksum, sbom_package_cpe_ref,
};
use uuid::Uuid;
...
async fn batch_describing_cpes(
    ...
    let rows = sbom_package_cpe_ref::Entity::find()
        .distinct()
        .select_only()
        .column(sbom_package_cpe_ref::Column::SbomId)
        .column(sbom_package_cpe_ref::Column::CpeId)
        .filter(sbom_package_cpe_ref::Column::SbomId.is_in(sbom_ids.to_vec()))
        .join(JoinType::Join, sbom_package_cpe_ref::Relation::Sbom.def())
        .join(
            JoinType::Join,
            sbom::Relation::PackageRelatesToPackages.def(),
