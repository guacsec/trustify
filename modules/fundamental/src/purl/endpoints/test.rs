...
async fn get_recommendations_other_status(ctx: &TrustifyContext) -> Result<(), anyhow::Error> {
    ...
    // Given a package with a vulnerability whose status is overridden to under_investigation
    ctx.graph
        .ingest_qualified_package(
            &Purl::from_str("pkg:cargo/hyper@0.14.1-redhat-00001")?,
            &ctx.db,
    ...
    for ps in purl_statuses {
        ...
        active.status = Set(Status::UnderInvestigation);
    ...
    assert_eq!(vuln["status"], "under_investigation");
