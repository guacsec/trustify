...
The active features are collected into a set at startup:

```rust
/// Active features for this instance. Presence in the set means the feature is available.
pub type ActiveFeatures = HashSet<Feature>;
...
At startup, each feature is added to the set only if its configuration is present and valid. The set is
registered as `web::Data<ActiveFeatures>`. When a Cargo feature is disabled at compile time, the
corresponding variant is never added — the code that would check the configuration is not compiled in.

#### Guard extractor

A `RequireFeature` extractor rejects requests when a feature is not in the active set:

```rust
pub struct RequireFeature<const GATE: Feature>;
```

Endpoint handlers declare the requirement in their signature:

```rust
#[get("/v3/purl/{key}/recommend")]
async fn recommend(
    _gate: RequireFeature<{ Feature::Recommendations }>,
...
This follows the `Require<Permission>` pattern already established in the codebase.
...
Both mechanisms — the `RequireFeature` extractor and the `features.require()` inline check — produce
